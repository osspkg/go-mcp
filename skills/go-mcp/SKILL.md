...
- Use `go.osspkg.com/mcp/client` when an application must consume a server:
  start `client.NewHTTPTransport`, `client.NewCommandTransport`,
  `client.NewStdioTransport`, or `client.NewSSETransport`, complete
  `Initialize`, then use the typed catalog and task helpers. Register
  `OnRequest` handlers for server-initiated roots, sampling, or elicitation
